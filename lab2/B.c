...
int partition_asc(int left, int right);
int partition_desc(int left, int right);
void quicksort_asc(int left, int right);
void quicksort_desc(int left, int right);
...
void quicksort_desc(int left, int right)
{
    if(left < right)
    {
        int q = partition_desc(left, right);
        quicksort_desc(left, q-1);
        quicksort_desc(q+1, right);
    }
}
...
void quicksort_asc(int left, int right)
{
    if(left < right)
    {
        int q = partition_asc(left, right);
        quicksort_asc(left, q-1);
        quicksort_asc(q+1, right);
    }
}
...
void inputter()
{
    ...
    while(i < 1000)
    {
        ...
        else if(strcmp(command,"PARTITION-ASC") == 0)
        {
            int left = atoi(word);
            int right = atoi(word2);
            partition_asc(left-1,right-1);
        }
        else if(strcmp(command,"QUICKSORT") == 0 && strcmp(word,"ASC") == 0)
        {
            quicksort_asc(0, heapSize);
        }
        else if(strcmp(command,"PARTITION-DESC") == 0)
        {
            int left = atoi(word);
            int right = atoi(word2);
            partition_desc(left-1,right-1);
        }
        else if(strcmp(command,"QUICKSORT") == 0 && strcmp(word,"DESC") == 0)
        {
            quicksort_desc(0, heapSize);
        }
        //else
        //{
        //    break;
...
{
    //FILE *in;
    //in = freopen("/Users/Brandon/Documents/2320 Algorithms/lab2/B/B/b.txt","r",stdin);
    inputter();
    //fclose(in);
    return 0;
}
